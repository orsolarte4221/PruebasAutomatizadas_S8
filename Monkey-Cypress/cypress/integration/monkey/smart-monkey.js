...
function avPag() {
    ...
    const pageHeight = Cypress.$('html').height();
  
    if (curPageMaxY - curY >= viewportHeight && curY + viewportHeight < pageHeight) {
...
function horizontalScrollFw(){
    ...
    if (curPageMaxX - curX >= viewportWidth) {
      if (curPageMaxX - (curX + viewportWidth) >= viewportWidth) {
        ...
        cy.window().then(($win) => {
          $win.scrollBy({ left: viewportWidth, top: 0, behavior: 'smooth' });
        });
      } else {
        curX = curPageMaxX - viewportWidth;
        cy.window().then(($win) => {
          $win.scrollTo({ left: curX, top: curY });
        });
        info += "Page limit reached! ";
      }
...
describe( `${appName} under smarter monkeys`, function() {
    ...
    it(`visits ${appName} and survives smarter monkeys`, function() {
        ...
        cy.login('s.mascareno@uniandes.edu.co','MISO123456789@')
