...
function getDataPool() {
  ...
  it('Hacer Login, Crear un post, publicarlo y consultar la publicación', () => {
     ...
     cy.get('textarea[name="post-setting-custom-excerpt"]').type(postData.excerpt_special_charts),{ parseSpecialCharSequences: false };     
      ...
      cy.get(properties.elements['post-title']).contains(postData.title,{ parseSpecialCharSequences: false }).should('exist');
  });    
});
