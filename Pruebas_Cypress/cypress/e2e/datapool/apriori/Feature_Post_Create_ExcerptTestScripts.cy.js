describe('Gestión Post', () => {
  ...
  it('Hacer Login, Crear un post, publicarlo y consultar la publicación', () => {
      ...
      cy.get(properties.elements['post-title-input']).type(postData.postexcerpt_testJavascript.title,{ parseSpecialCharSequences: false });
 
      cy.wait(3000);
      cy.get(properties.elements['post-title-input']).type('{enter}',{overwrite: true},{capture: 'runner'});
     //add special data
     cy.get(".koenig-editor__editor").type(postData.postexcerpt_testJavascript.body,{ parseSpecialCharSequences: false });
     ...
     cy.get('textarea[name="post-setting-custom-excerpt"]').type(postData.postexcerpt_testJavascript.excerpt_javascript);     
      ...
      cy.get(properties.elements['post-title']).contains(postData.postexcerpt_testJavascript.title,{ parseSpecialCharSequences: false }).should('exist');
  });    
});
