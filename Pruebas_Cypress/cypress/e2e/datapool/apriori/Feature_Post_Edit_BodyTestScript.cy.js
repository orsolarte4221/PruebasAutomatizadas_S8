describe('Gestión Post', () => {
  ...
  it('Hacer Login, Crear Post, Editar un post, Publicar Post y Validar publicación', () => {
      ...
      cy.wait(3000);
      cy.get(properties.elements["post-title-input"]).clear().type(postData.postbody_testJavascript.title);
     ...
     cy.screenshot(`${test_name}/post_edit_body_special_chart`,{overwrite: true},{capture: 'runner'})      
      ...
      cy.wait(3000);
      cy.get(properties.elements["post-title"]).contains(postData.postbody_testJavascript.title).should('exist');
