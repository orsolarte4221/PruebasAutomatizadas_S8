describe('Gestión Post', () => {
  ...
  it('Hacer Login, Crear un post, publicarlo y consultar la publicación', () => {
      ...
      cy.get(properties.elements['post-title-input']).type(postData.postbody_testJavascript.title,{ parseSpecialCharSequences: false });
 
      cy.wait(3000);
      cy.get(properties.elements['post-title-input']).type('{enter}',{overwrite: true},{capture: 'runner'});
     //add special data
     cy.get(".koenig-editor__editor").type(postData.postbody_testJavascript.body,{ parseSpecialCharSequences: false });
     cy.screenshot(`${test_name}/post_edit_body_special_chart`,{overwrite: true},{capture: 'runner'})
      ...
      cy.get(properties.elements['post-title']).contains(postData.postbody_testJavascript.title,{ parseSpecialCharSequences: false }).should('exist');
  });    
});
